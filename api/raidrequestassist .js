...
export default async function handler(req, res) {
  ...
  if (data.assist_used)    return res.status(403).json({ error: "이미 어시스트를 사용했음" })
  if (data.assist_active)  return res.status(403).json({ error: "어시스트가 이미 활성화됨" })
