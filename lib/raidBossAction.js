...
/**
 * [FIX] processHealCommand: 회복 후 anyAbove50 리턴
 * raidBossAction에서 이 값으로 nextState.step을 결정 (defend or attack)
 */
...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  const {
    ...
    // [FIX] getBossIntroLogs (복수, 배열 리턴) 으로 통일
    getBossIntroLogs,
  ...
  if (isStatusBlocked) {
    ...
    // [FIX] recharge 양 턴 모두 commandLog 찍음 (beequeen이 각 턴에 맞는 log 리턴함)
    if (commandLog) logEntries.push(makeLog("normal", commandLog))
    ...
    // [FIX] heal 결과에 따라 nextState.step 결정
    // 50% 이상 회복됐으면 다음 턴 방어지령, 아니면 공격지령
  ...
  // ── 독침붕 전멸 체크 ────────────────────────────────────────────
  // [FIX] "attack" command(공격지령)도 포함 — 독침붕이 이미 플레이어 턴에 죽었을 수도 있음
  // summon/recharge/direct 제외하고 모두 체크
  if (command !== "direct" && command !== "recharge" && command !== "summon") {
    ...
    if (deathCheck.allDead) {
      ...
      // killCount 누적은 checkBeedrilDeath 내부에서 처리
      // 3번 이상이면 2페이즈 진입 (checkPhase2Enter 훅이 처리)
