...
function processAttackCommand(data, entries, targetSlot, beedrillLog, logEntries) {
  ...
  const alive = getAlivePlayers(data, entries)
  if (alive.length === 0) { logEntries.push(makeLog("normal", "공격할 대상이 없다!")); return }
  const realTarget = targetSlot && alive.includes(targetSlot)
    ? targetSlot
...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  const {
    ...
    getBossIntroLogs, getPhase2EnterLogs,
  ...
  // ── 앱솔 HP 70% 이하 최초 진입 체크 ───────────────────────────
  // getPhase2EnterLogs가 있고(앱솔), 아직 phase2EnterLogged가 없는 경우
  if (getPhase2EnterLogs && !data.boss_state?.phase2EnterLogged) {
    const hpRatioNow = (data.boss_current_hp ?? 0) / (data.boss_max_hp ?? 1)
    if (hpRatioNow <= 0.7) {
      for (const text of getPhase2EnterLogs()) {
        logEntries.push(makeLog("normal", text))
      }
      // nextState에 반영은 아래 decideBossMove 이후 병합
      data.boss_state = { ...(data.boss_state ?? {}), phase2EnterLogged: true }
    }
  }
  ...
  if (shouldTriggerUlt(data)) {
    ...
    if (ultTarget) {
      ...
      // 기습 공격 (move_announce는 processBossAttack 내부에서 찍히므로 생략 가능)
  ...
  // phase2EnterLogged 플래그 유지
  if (data.boss_state?.phase2EnterLogged) {
  ...
  if (command !== "direct" && command !== "recharge" && command !== "summon") {
    ...
    if (deathCheck.allDead) {
      ...
      if (nextState.beedrillKillCount < 3) {
  ...
  // ── 비퀸 HP 60% 이하 2페이즈 진입 체크 ─────────────────────────
  const hpRatio = (data.boss_current_hp ?? 0) / (data.boss_max_hp ?? 1)
  if (
    hpRatio <= 0.6 &&
    command !== "direct" &&
    data.boss_state?.step !== "recharge" &&
    data.boss_state?.step !== "recharge2"
  ) {
    const wasPhase1 = !nextState.phase2Step
    if (wasPhase1) {
      // 비퀸 2페이즈 진입 순차 로그
      if (getPhase2EnterLogs && bossName !== "앱솔") {
        for (const text of getPhase2EnterLogs()) {
          logEntries.push(makeLog("normal", text))
        }
      }
  ...
  if (bossJustDied && getDeathLogs) {
    const deathLogs = getDeathLogs()
    for (const text of deathLogs) {
      logEntries.push(makeLog("normal", text))
    }
  }
