...
async function processSummon(roomId, data, logEntries) {
  ...
  data.boss_beedrills = [beedrill1, beedrill2]
  logEntries.push(makeLog("normal", "비퀸이 독침붕을 두 마리 소환했다!"))
  logEntries.push(makeLog("beedrill_summon", "", { beedrills: data.boss_beedrills }))
}
...
function processHealCommand(data, logEntries) {
  ...
  for (const bee of beedrills) {
    ...
    logEntries.push(makeLog("beedrill_hp", "", { beedrills: data.boss_beedrills }))
