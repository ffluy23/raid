...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  // ── 마비/얼음 상태이상 체크 (kangaskhan_dual 은 제외 — 내부서 처리) ─
  ...
  // ── [캥카] 듀얼 턴 처리 ─────────────────────────────────────────
    ...
    if (processKangaskhanTurn) {
      ...
      // 수정: processKangaskhanTurn이 data.boss_state를 직접 변경(2페이즈 전환 등)하므로
      // nextState를 data.boss_state로 동기화해서 Firestore에 올바르게 반영
  ...
  // ── 독침붕 전멸 체크 ────────────────────────────────────────────
  if (command !== "direct" && command !== "recharge" && command !== "summon" && command !== "kangaskhan_dual") {
