...
function processHealCommand(data, logEntries, beedrillLog) {
  ...
  for (const bee of beedrills) {
    ...
    bee.wasHealed = true   // 6-6 로그용 플래그
...
export async function executeBossAction(roomId, data, entries, currentOrder, extraUpdate = {}) {
  ...
  // ── 독침붕 대기 로그 (독침붕 턴 — 보스 턴 직전에 출력) ──────────
  // current_order[0] === "boss" 이전에 독침붕이 살아있으면 대기 로그
  ...
  // ── 비퀸 지령 처리 ─────────────────────────────────────────────
    ...
    // commandLog = recharge첫턴:독침붕퇴장 / 두번째턴:비축
    if (commandLog) logEntries.push(makeLog("normal", commandLog))
    if (nextState.step === "summon") {
      // recharge2 → summon 전환 시 바로 소환
      await processSummon(roomId, data, logEntries, null)
    }
