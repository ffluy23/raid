...
/**
 * 2페이즈 진입 체크 훅
 * HP 60% 이하이고 아직 phase2Step이 없을 때 진입
 */
export function checkPhase2Enter(data, nextState, command) {
  if (command === "direct" || command === "recharge" || command === "recharge2") return null
  if (nextState.phase2Step) return null
  if (data.boss_state?.step === "recharge" || data.boss_state?.step === "recharge2") return null

  const hpRatio = (data.boss_current_hp ?? 0) / (data.boss_max_hp ?? 1)
  if (hpRatio > 0.6) return null
