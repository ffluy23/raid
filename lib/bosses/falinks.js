...
export function applyInheritedRanks(data, inheritedRanks) {
  ...
  const r = data.boss_rank ?? { atk: 0, atkTurns: 0, def: 0, defTurns: 0, spd: 0, spdTurns: 0 }
  const ATK_MAX = 4, DEF_MAX = 3, SPD_MAX = 5
  data.boss_rank = {
    atk:      Math.min(ATK_MAX, (r.atk ?? 0) + inheritedRanks.atk),
    atkTurns: 999,
    def:      Math.min(DEF_MAX, (r.def ?? 0) + inheritedRanks.def),
    defTurns: 999,
    spd:      Math.min(SPD_MAX, (r.spd ?? 0) + inheritedRanks.spd),
    spdTurns: 999,
  }
}