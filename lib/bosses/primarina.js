...
export function decideBossMove(data, entries, playerSlots) {
  ...
  if (phase === 1 && !state.dropletUsed) {
    const ratio = (data.boss_current_hp ?? 0) / (data.boss_max_hp ?? 1)
    if (ratio <= 0.80) {
      // 물방울 사용 후 유혹 중인 대상이 있으면 step 1(하이드로펌프)부터, 없으면 step 0(차밍보이스)부터
      const seduced = getSeducedSlots(data, entries)
      const nextLoopStep = seduced.length > 0 ? 1 : 0
      return {
        command:    "direct",
        moveName:   "생명의물방울",
        targetSlot: null,
        log:        `${bossName}${josa(bossName, "은는")} 생명의물방울로 체력을 회복했다!`,
        nextState:  { ...state, dropletUsed: true, loopStep: nextLoopStep },
      }
    }
  }
