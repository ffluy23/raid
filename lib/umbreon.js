...
import { makeLog, defaultRanks, PLAYER_SLOTS } from "./raidBossAction.js"

const UMBREON_HP_THRESHOLD = 0.4  // 40% 이하
const UMBREON_REVENGE_MULT = 2    // 보복 배율

// ── 블래키 발동 처리 ─────────────────────────────────────────────────
// damages: applySyncDistribution 결과 { [slot]: dmg }
// damages 객체를 직접 수정 → 차단된 슬롯을 0으로 만듦
// 반환: { activated: boolean }
export function activateUmbreon(damages, data, entries, logEntries) {
  // 이미 사용했으면 발동 안 함
  if (data.umbreon_used) return { activated: false }

  // damages 중 조건 만족하는 슬롯 탐색
  // - 마지막 포켓몬 (벤치에 살아있는 포켓몬 없음)
  // - 현재 출전 포켓몬 HP 40% 이하
  ...
  // ── 등장 연출 ──────────────────────────────────────────────────────
  logEntries.push(makeLog("umbreon", ""))  // Override! 애니메이션 트리거
  ...
  // ── 피해 차단 (해당 슬롯 데미지를 0으로) ──────────────────────────
  damages[triggerSlot] = 0

  // ── 보복 공격 ──────────────────────────────────────────────────────
  const revengeRawDmg = blockedDamage * UMBREON_REVENGE_MULT
  const finalRevenge  = Math.max(1, revengeRawDmg)
