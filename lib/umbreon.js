// lib/umbreon.js
// 블래키 보호 시스템
// 조건: 세 플레이어 중 한 명이라도 마지막 포켓몬 + (HP 40% 이하 OR 이번 공격으로 기절)일 때
...
export function activateUmbreon(damages, data, entries, logEntries) {
  ...
  for (const [slot, dmg] of Object.entries(damages)) {
    ...
    const wouldFaint = activePkmn.hp - dmg <= 0

    // HP 40% 이하이거나, 이번 공격으로 기절하는 경우 발동
    if (hpRatio > UMBREON_HP_THRESHOLD && !wouldFaint) continue
  ...
  data.boss_state = {
    ...data.boss_state,
    dealCheckDmg: (data.boss_state.dealCheckDmg ?? 0) + finalRevenge,
  }
}
