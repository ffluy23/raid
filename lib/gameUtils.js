...
export function buildEntryUpdate(entries) {
  const u = {}
  ALL_FS.forEach(s => { u[`${s}_entry`] = entries[s] })
  return u
}
...
export async function handleEot(db, roomId, entries, data, update) {
  ...
  update.pending_switches = collectFaintedSlots(entries, data)
  return null
}